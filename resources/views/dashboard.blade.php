<x-app-layout>
    <div class="bg-slate-900 min-h-screen pb-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
            ...
            <!-- 인사말 -->
            <div class="mb-8">
                <h1 class="text-2xl sm:text-3xl font-bold text-white">{{ Auth::user()->profile?->display_name ?? '학습자' }}님, 오늘의 학습</h1>
                <p class="text-slate-400 mt-1">꾸준히 한 걸음씩 나아가고 있어요.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <span class="text-sm text-slate-400">등록 트랙</span>
                    </div>
                    <div class="text-3xl font-bold text-amber-400">{{ $enrollments->count() }}</div>
                </div>
                <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <span class="text-sm text-slate-400">복습 대기</span>
                    </div>
                    <div class="text-3xl font-bold text-amber-400">{{ $reviewDueCount }}</div>
                </div>
                <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <span class="text-sm text-slate-400">완료 레슨</span>
                    </div>
                    <div class="text-3xl font-bold text-amber-400">{{ $recentAttempts->where('status', 'completed')->count() }}</div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-amber-500/10 to-yellow-500/10 border border-amber-500/20 rounded-2xl p-6 mb-10 flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h3 class="text-white font-semibold">복습할 카드가 {{ $reviewDueCount }}개 있어요</h3>
                    <p class="text-slate-400 text-sm mt-1">잊기 전에 복습하면 기억이 오래 갑니다.</p>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <!-- 진행 중인 트랙 -->
                <div>
                    <h2 class="text-lg font-semibold text-white mb-4">진행 중인 트랙</h2>
                    @forelse($enrollments as $enrollment)
                    <a href="{{ route('tracks.show', $enrollment->learningTrack->slug) }}"
                       class="block bg-white/5 border border-white/10 rounded-2xl p-5 mb-3 hover:border-amber-500/30 transition-all group">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-white font-medium group-hover:text-amber-400 transition-colors">{{ $enrollment->learningTrack->title }}</h3>
                            <span class="text-amber-400 text-sm font-bold">{{ number_format($enrollment->progress_percent, 0) }}%</span>
                        </div>
                        ...
                        <p class="text-xs text-slate-500 mt-2">마지막 학습: {{ $enrollment->last_accessed_at->diffForHumans() }}</p>
                        @endif
                    </a>
                    @empty
                    <div class="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
                        <p class="text-slate-500 mb-4">아직 시작한 트랙이 없어요.</p>
                ...
                <!-- 최근 학습 -->
                <div>
                    <h2 class="text-lg font-semibold text-white mb-4">최근 학습</h2>
                    @forelse($recentAttempts->take(5) as $attempt)
                    <div class="flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl p-4 mb-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-white truncate">{{ $attempt->lesson->title }}</p>
                            <p class="text-xs text-slate-500">{{ $attempt->last_accessed_at?->diffForHumans() ?? '' }}</p>
                        </div>
                    </div>
                    @empty
                    <div class="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
                        <p class="text-slate-500">아직 학습 기록이 없어요.</p>
                    </div>
                    @endforelse
                </div>
            </div>
