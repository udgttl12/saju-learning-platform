    ...
    <div class="min-h-screen pb-16">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-8">
            <div class="text-center">
                <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-2">학습 여정</h1>
                <p class="text-gray-600 dark:text-slate-400">총 {{ $tracks->count() }}개 트랙 · {{ count($enrolledTrackIds) }}개 진행 중</p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @forelse($tracks as $i => $track)
                    ...
                    <a href="{{ route('tracks.show', $track->slug) }}"
                       ...
                       {{ $enrolled ? 'border-amber-500/40' : 'border-gray-200 dark:border-white/10' }}">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="flex items-center gap-2 mb-4">
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium
                                    {{ $track->difficulty_level <= 1 ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : '' }}
                                    {{ $track->difficulty_level == 2 ? 'bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400' : '' }}
                                    {{ $track->difficulty_level >= 3 ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400' : '' }}">
                        ...
                        <!-- 화살표 -->
                        <div class="absolute bottom-6 right-6 text-gray-400 dark:text-slate-600 group-hover:text-amber-400 transition-colors">
                ...
                @empty
                    <div class="col-span-full text-center py-16">
                        <p class="text-gray-500 dark:text-slate-500 text-lg">아직 공개된 학습 트랙이 없습니다.</p>
                    </div>
                @endforelse
            </div>
        </div>
